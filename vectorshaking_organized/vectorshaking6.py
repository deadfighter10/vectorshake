...
def research(name, column_mapping=None, start=1, end=10):
    ...
    for i in range(start, end+1):
        ...
        baseline_time.append(datetime.datetime.now() - time1)
        ...
        setfit_time.append(datetime.datetime.now() - time2)
        setfit_data.append(acc["accuracy"])
        #trainer.search_parameters(trials=1)
        trainer.train()
        trainer.evaluate()
        time3 = datetime.datetime.now()
        trainer.train()
        acc = trainer.evaluate()["accuracy"]
        vectorshake_time.append(datetime.datetime.now() - time3)
        vectorshake_data.append(acc)
        # Save the results to a file
        with open(f"results/results_temp_{dataset_name}.txt", "w") as f:
            f.write(str(baseline_data) + "\n")
            f.write(str(setfit_data) + "\n")
            f.write(str(vectorshake_data) + "\n")
            f.write(str(baseline_time) + "\n")
            f.write(str(setfit_time) + "\n")
            f.write(str(vectorshake_time) + "\n")

    with open(f"results/results_{dataset_name}.txt", "w") as f:
        f.write(str(baseline_data) + "\n")
        f.write(str(setfit_data) + "\n")
        f.write(str(vectorshake_data) + "\n")
        f.write(str(baseline_time) + "\n")
...
def ablation_study(dataset_name):
    column_mapping = {"text": "text", "label": "coarse_label"}
    trainer = Trainer(dataset_name=dataset_name, K_class=5, epochs=30, seed=42, args=args, column_mapping=column_mapping)
