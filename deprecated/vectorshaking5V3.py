...
# Choose dataset here:
DATASET_NAME = "semeval"  # Change to "go_emotions" to switch
...
# ------------------------------------------------------------------------------
def train_improved(model, data_loader, optimizer, device, lambda_mix, lambda_contrast, lambda_consistency, lambda_reconstruct, beta_a,
                   beta_b, epsilon):
    model.train()
    total_loss = 0.0
    ...
    for batch in data_loader:
        ...
        # Reconstruction Loss
        reconstructed_latent = model.reconstruct(clean_latent)
        loss_reconstruct = F.mse_loss(reconstructed_latent, clean_latent)  # Or use F.cosine_similarity
        ...
        total_batch_loss = loss_ce + \
                           lambda_reconstruct * loss_reconstruct + \
                           lambda_mix * loss_mix + \
...
def objective(trial: Trial):
    console.rule(f"[bold blue] Trial starting: {trial.number} [/bold blue]")
    set_seed(42)
    ...
    lambda_reconstruct = trial.suggest_float("lambda_reconstruct", 0.01, 1.0)  # Example range
    ...
    history = []
    num_epochs_opt = 20
    with Live(console=console, refresh_per_second=10) as live:  # Use Live for dynamic display
        for epoch in range(1, num_epochs_opt + 1):
            ...
            # Create a table to display the progress information
            table = Table()
            ...
            progress_percent = (epoch / num_epochs_opt) * 100
            progress_bar = "[green]" + "#" * int(progress_percent) + "-" * (100 - int(progress_percent)) + "[/]"

            table.add_row(
                f"{epoch}/{num_epochs_opt}",
                f"{train_loss:.4f}",
                f"{val_acc:.4f}%",
    ...
    trial.set_user_attr("lambda_reconstruct", lambda_reconstruct)
...
def logging_callback(study, trial):
    console.print(f"Params: {trial.params}\n\n\nVal Accuracy = {trial.value:.4f}\nBest: {study.best_value:.4f}\n")
...
def main():
    ...
    if USE_OPTIMIZATION:
        ...
        study = optuna.create_study(direction="maximize", storage=storage_name, study_name="SemEval_test6",
                                    load_if_exists=True)
    ...
    with Live(console=console, refresh_per_second=10) as live:  # Use Live for dynamic display
        for epoch in range(1, num_epochs + 1):
            train_loss = train_improved(improved_model, train_loader, optimizer_improved, device,
                                        ...
                                        lambda_reconstruct=best_params["lambda_reconstruct"],
