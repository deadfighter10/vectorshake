...
# Process each example: choose the emotion with the highest score.
...
# Stratified sampling: use 10% of the training data.
...
# ------------------------------------------------------------------------------
# 6. Improved Training Function: Combined Latent Manifold Mixup & Contrastive Loss
# ------------------------------------------------------------------------------
def train_improved(model, data_loader, optimizer, device, lambda_mix, lambda_contrast, beta_a, beta_b, epsilon):
    ...
    For each batch:
      ...
      - Compute two adversarial perturbations to obtain two different views.
      - Compute the original classification loss on unperturbed logits.
      - Compute a contrastive loss between the two adversarial views (using 1 - cosine similarity).
      - Perform latent manifold mixup on one of the adversarial views:
           * Convert labels to one-hot.
           * Sample a mixup coefficient alpha from a Beta distribution.
           * Shuffle the batch and interpolate the perturbed latents and one-hot labels.
           * Compute a mixup loss (via KL divergence) on the mixed prediction.
      - Combine the original loss, the weighted mixup loss, and the weighted contrastive loss.
    """
    model.train()
    total_loss = 0.0
    for batch in data_loader:
        ...
        latent = outputs.hidden_states[-1][:, 0, :]  # [CLS] token from unperturbed data
        ...
        # Contrastive loss: encourage same-sample views to be similar.
        # We use 1 - cosine similarity as the loss.
        ...
        alpha = torch.distributions.Beta(beta_a, beta_b).sample((adv_view1.size(0),)).to(device)
        alpha = alpha.unsqueeze(1)
        indices = torch.randperm(adv_view1.size(0))
        adv_view1_shuffled = adv_view1[indices]
        labels_shuffled = labels_onehot[indices]
        mixed_latent = alpha * adv_view1 + (1 - alpha) * adv_view1_shuffled
...
def objective(trial: Trial):
    # Suggest hyperparameters for mixup and contrastive loss
    lambda_mix = trial.suggest_float("lambda_mix", 0.1, 3.0)
    lambda_contrast = trial.suggest_float("lambda_contrast", 0.1, 3.0)
    beta_a = trial.suggest_float("beta_a", 0.5, 1.0)
    beta_b = trial.suggest_float("beta_b", 0.5, 1.0)
...
# Note: We update the objective to call our new train_improved function.
def train_improved(model, data_loader, optimizer, device, lambda_mix, lambda_contrast, beta_a, beta_b, epsilon):
    # Our improved training function combining mixup and contrastive loss.
    model.train()
    total_loss = 0.0
    for batch in data_loader:
        set_seed(42)
        ...
        # Contrastive loss: 1 - cosine similarity for each sample
        cos_sim = F.cosine_similarity(adv_view1, adv_view2, dim=1)
        loss_contrast = torch.mean(1 - cos_sim)
