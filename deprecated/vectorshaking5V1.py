...
# ------------------------------------------------------------------------------
NUM_EPOCHS = 20  # Number of epochs for both optimization and final training
USE_OPTIMIZATION = True  # Set to False to load hyperparameters from JSON
OPTIMIZATION_FILE = "best_params.json"

# Dataset configuration dictionary allows easy switching between datasets.
DATASET_CONFIGS = {
    "semeval": {
...
DATASET_NAME = "go_emotions"  # Change to "go_emotions" if needed
...
# ------------------------------------------------------------------------------
# 1. Load and Process Dataset (SemEval-2018 Task 1, Subtask5.english)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 6. Improved Training Function: Combined FGSM, Mixup & Contrastive Loss
# ------------------------------------------------------------------------------
def train_improved(model, data_loader, optimizer, device, lambda_mix, lambda_contrast, beta_a, beta_b, epsilon):
    ...
    for batch in data_loader:
        ...
        # Generate two adversarial views using FGSM
        adv_view1 = fgsm_adversarial_perturbation(latent, labels, model, epsilon)
        adv_view2 = fgsm_adversarial_perturbation(latent, labels, model, epsilon)
...
def objective(trial: Trial):
    lambda_mix = trial.suggest_float("lambda_mix", 0.2, 4.0)
    lambda_contrast = trial.suggest_float("lambda_contrast", 0.2, 4.0)
    beta_a = trial.suggest_float("beta_a", 0.5, 5.0)
    beta_b = trial.suggest_float("beta_b", 0.8, 6.0)
    epsilon = trial.suggest_float("epsilon", 0.002, 0.2)
...
def main():
    ...
    if USE_OPTIMIZATION:
        ...
        storage_name = "sqlite:///optuna_study_goemotions.db"
        study = optuna.create_study(direction="maximize", storage=storage_name, study_name="GoEmotions_Study", load_if_exists=True)
    ...
    console.rule("[bold blue]Final Training: Improved Model with FGSM, Mixup & Contrastive Loss")
    config = BertConfig.from_pretrained("bert-base-uncased", num_labels=num_labels)
    config.output_hidden_states = True
    improved_model = BertForSequenceClassificationWithReconstruction.from_pretrained("bert-base-uncased", config=config)
    improved_model.to(device)
    optimizer_improved = AdamW(improved_model.parameters(), lr=best_params["lr"],
    ...
    optimizer_baseline = AdamW(baseline_model.parameters(), lr=best_params["lr"],
                               weight_decay=best_params["weight_decay"])
    ...
    # Save final experiment results in JSON file
    final_results = {
