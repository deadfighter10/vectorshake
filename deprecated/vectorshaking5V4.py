...
def dynamic_curriculum(epoch_stats, params, old_acc, new_acc, old_mixup_loss, mixup_loss, strength=0.1, acc_threshold=0.004, mixup_threshold=0.004):
    new_params = params.copy()

    # Calculate deltas
    acc_delta = new_acc - old_acc
    mixup_delta = old_mixup_loss - mixup_loss  # Positive if mixup_loss improved

    # Update epsilon based on accuracy changes
    if acc_delta > acc_threshold:
        # Increase adversarial strength proportionally, with a max cap to prevent instability
        factor = 1 + strength * (acc_delta / acc_threshold)
        new_params["epsilon"] = min(new_params["epsilon"] * factor, 10)
    elif acc_delta < -acc_threshold:
        # Decrease epsilon if accuracy has dropped significantly, with a min floor
        factor = 1 - strength * (abs(acc_delta) / acc_threshold)
        new_params["epsilon"] = max(new_params["epsilon"] * factor, 0.0001)

    # Update mixup parameters based on mixup loss improvement
    if mixup_delta > mixup_threshold:
        # Better mixup performance: encourage stronger mixing and more aggressive augmentation
        factor = 1 + strength * (mixup_delta / mixup_threshold)
        new_params["lambda_mix"] = min(new_params["lambda_mix"] * factor, 7.0)
        new_params["beta_a"] = min(new_params["beta_a"] * factor, 8.0)
        new_params["beta_b"] = min(new_params["beta_b"] * factor, 9.0)
    elif mixup_delta < -mixup_threshold:
        # Worsening mixup loss: reduce the augmentation strength
        factor = 1 - strength * (abs(mixup_delta) / mixup_threshold)
        new_params["lambda_mix"] = max(new_params["lambda_mix"] * factor, 0.05)
        new_params["beta_a"] = max(new_params["beta_a"] * factor, 0.01)
        new_params["beta_b"] = max(new_params["beta_b"] * factor, 0.01)
...
def objective(trial: optuna.Trial):
    ...
    params = {
        ...
        "epsilon": trial.suggest_float("epsilon", 0.005, 2),
        "lr": trial.suggest_float("lr", 1e-7, 1e-3, log=True),
        "weight_decay": trial.suggest_float("weight_decay", 0.0, 0.1),
        "DAC_strength": trial.suggest_float("DAC_strength", 0.005, 0.15),
        "acc_threshold": trial.suggest_float("acc_threshold", 0.00001, 0.05),
    ...
    for epoch in range(Config.NUM_EPOCHS):  # Short optimization run
        ...
        params = dynamic_curriculum(
            epoch_stats=stats,
            params=params,
            old_acc=old_acc,
            new_acc=metrics["accuracy"],
            old_mixup_loss=old_mixup_loss,
            mixup_loss=stats['mix_loss'],
...
def main():
    ...
    with Progress() as progress:
        ...
        for epoch in range(Config.NUM_EPOCHS):
            ...
            # Dynamic curriculum update
            current_params = dynamic_curriculum(
                epoch_stats=train_stats,
                params=current_params,
                old_acc=old_acc,
                new_acc=val_acc,
                old_mixup_loss=old_mixup_loss,
                mixup_loss=train_stats['mix_loss'],
            ...
            old_acc = val_acc
            old_mixup_loss = train_stats['mix_loss']
