...
# Rich for advanced, fixed progress display
...
# 1. Dataset Class
# ------------------------------
# We convert the Hugging Face dataset splits to lists of texts and labels.
class SentenceDataset(Dataset):
    def __init__(self, texts, labels, tokenizer, max_length=128):
        self.texts = texts
        self.labels = [int(label[0]) for label in labels]  # For SST-2, labels are 0 or 1.
...
class BertForSequenceClassificationWithReconstruction(BertForSequenceClassification):
    def __init__(self, config, **kwargs):
        ...
        # Reconstruction head: a linear layer to project the perturbed latent vector back to the original latent space.
...
def adversarial_perturbation(latent, labels, model, epsilon):
    # Detach and clone to create a leaf tensor and set requires_grad.
...
def train_improved_multi(model, data_loader, optimizer, device, current_epoch, total_epochs,
    ...
    for batch in data_loader:
        ...
        # Generate multiple (num_variations) perturbed variations
...
# ------------------------------
# 6. Main Experiment Function with Hugging Face Dataset
# ------------------------------
...
def main():
    ...
    # Load the GLUE SST-2 dataset from Hugging Face
    dataset = load_dataset("go_emotions")
    train_texts = dataset["test"]["text"]
    train_labels = dataset["test"]["labels"]
    train_texts, val_texts, train_labels, val_labels = train_test_split(train_texts, train_labels, test_size=0.8, random_state=42)
    val_texts = dataset["validation"]["text"]
    val_labels = dataset["validation"]["labels"]

    # Create our custom dataset objects
