...
class TrainingLogger:
    ...
    def _safe_config(self):
        """Convert configuration to JSON-safe types"""
        safe = {}
    ...
    def log(self, phase, data):
        self.log_data[phase].append(data)
        self._save()
...
def train_baseline(model, loader, optimizer, device, logger):
    ...
    logger.log("baseline", {"train_loss": avg_loss})
    return avg_loss


def train_improved(model, loader, optimizer, device, params, logger, epoch):
    set_seed(Config.SEED)
...
import sys # Needed for float_info.min for initialization comparison
...
def dynamic_curriculum_ema(
    ...
    in accuracy and mixup loss over epochs.
    ...
    # Handle initialization on the first epoch/call
    is_first_epoch = (prev_ema_acc < 0 or prev_ema_mixup_loss == sys.float_info.max) # Example sentinel check

    if is_first_epoch:
        current_ema_acc = current_acc
        current_ema_mixup_loss = current_mixup_loss
        ema_acc_delta = 0  # No change on the first epoch
        ema_mixup_delta = 0 # No change on the first epoch
    else:
        # Apply EMA formula
        current_ema_acc = ema_alpha * current_acc + (1 - ema_alpha) * prev_ema_acc
        current_ema_mixup_loss = ema_alpha * current_mixup_loss + (1 - ema_alpha) * prev_ema_mixup_loss

        # Calculate deltas based on EMA changes
        ema_acc_delta = current_ema_acc - prev_ema_acc
        ema_mixup_delta = prev_ema_mixup_loss - current_ema_mixup_loss  # Positive if EMA mixup_loss improved

    # --- Update parameters based on EMA deltas ---

    # Update epsilon based on EMA accuracy changes
    if ema_acc_delta > acc_threshold:
        # Increase adversarial strength proportionally, with a max cap
        factor = 1 + strength * (ema_acc_delta / acc_threshold)
        new_params["epsilon"] = min(new_params["epsilon"] * factor, 100)
        # print(f"Epoch EMA Acc Delta: {ema_acc_delta:.4f} > {acc_threshold:.4f}. Increasing epsilon by factor {factor:.3f} to {new_params['epsilon']:.4f}")
    elif ema_acc_delta < -acc_threshold:
        # Decrease epsilon if EMA accuracy has dropped significantly, with a min floor
        factor = 1 - strength * (abs(ema_acc_delta) / acc_threshold)
        new_params["epsilon"] = max(new_params["epsilon"] * factor, 0.0001)
        # print(f"Epoch EMA Acc Delta: {ema_acc_delta:.4f} < {-acc_threshold:.4f}. Decreasing epsilon by factor {factor:.3f} to {new_params['epsilon']:.4f}")

    # Update mixup parameters based on EMA mixup loss improvement
    if ema_mixup_delta > mixup_threshold:
        # Better mixup performance (EMA loss decreased): encourage stronger mixing
        factor = 1 + strength * (ema_mixup_delta / mixup_threshold)
        new_params["lambda_mix"] = min(new_params["lambda_mix"] * factor, 7.0)
        new_params["beta_a"] = min(new_params["beta_a"] * factor, 10.0)
        new_params["beta_b"] = min(new_params["beta_b"] * factor, 10.0)
        # print(f"Epoch EMA Mixup Delta: {ema_mixup_delta:.4f} > {mixup_threshold:.4f}. Increasing mixup params by factor {factor:.3f}")

    elif ema_mixup_delta < -mixup_threshold:
        # Worsening mixup loss (EMA loss increased): reduce the augmentation strength
        factor = 1 - strength * (abs(ema_mixup_delta) / mixup_threshold)
        new_params["lambda_mix"] = max(new_params["lambda_mix"] * factor, 0.05)
        new_params["beta_a"] = max(new_params["beta_a"] * factor, 0.0001)
        new_params["beta_b"] = max(new_params["beta_b"] * factor, 0.0001)
        # print(f"Epoch EMA Mixup Delta: {ema_mixup_delta:.4f} < {-mixup_threshold:.4f}. Decreasing mixup params by factor {factor:.3f}")
...
def objective(trial: optuna.Trial):
    ...
    params = {
        ...
        "lambda_reconstruct": trial.suggest_float("lambda_reconstruct", 0.01, 4.0),
        "beta_a": trial.suggest_float("beta_a", 0.1, 5.0),
        "beta_b": trial.suggest_float("beta_b", 0.1, 6.0),
        "epsilon": trial.suggest_float("epsilon", 0.005, 1),
        "lr": trial.suggest_float("lr", 1e-6, 1e-3, log=True),
        "weight_decay": trial.suggest_float("weight_decay", 0.0, 0.1),
        "DAC_strength": trial.suggest_float("DAC_strength", 0.01, 0.2),
        "acc_threshold": trial.suggest_float("acc_threshold", 0.0001, 0.05),
        "mixup_threshold": trial.suggest_float("mixup_threshold", 0.00001, 0.1),
        "ema_alpha": trial.suggest_float("ema_alpha", 0.01, 2)
    }
    ...
    for epoch in range(Config.NUM_EPOCHS):  # Short optimization run
        stats = train_improved(model, train_loader, optimizer, device, params, TrainingLogger(), epoch)
    ...
    if console.input("Train baseline? (y/n): ").lower() == "y":
        ...
        with Progress() as progress:
            ...
            for epoch in range(Config.NUM_EPOCHS):
                train_loss = train_baseline(baseline_model, train_loader, optimizer, device, logger)
